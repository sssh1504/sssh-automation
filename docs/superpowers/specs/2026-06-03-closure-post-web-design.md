...
則自動把公文發佈到松高校網（www.sssh.tp.edu.tw）的**圖書館頁「圖書館公告」模組**：
...
maybe_post_announcement(driver, extract_dir)
  ...
  ├─[6] _submit_announcement(driver, title, body)  到 圖書館頁「圖書館公告」模組(先進「模組」編輯模式) 填表「發布」
  ├─[7] 驗證發佈成功
...
### (c) 發佈 `_submit_announcement(driver, title, body, attachments, sync_keyword)` — 圖書館公告（2026-06-06 實機驗證）

目標板：**圖書館頁的「圖書館公告」模組** = `https://www.sssh.tp.edu.tw/nss/s/main/p/library`（`ANNO_URL`）。
⚠️ 該模組的「新增公告」鈕**只在 NSS「模組」編輯模式才出現**；直接導覽看不到。
（先前誤用 `/NetworkCenterAnnoucement`——那是另一個模組「公告訊息取代檔案上傳系統TEST」，非圖書館公告。）

1. 導到 `ANNO_URL`。若頁面無「新增公告」鈕 → 點上方 admin bar 的「**模組**」進編輯模式（JS click）。
   再 **輪詢等**「新增公告」鈕出現（最多 ~10s）→ 點它（`button` text == 「新增公告」，JS click）。
2. 開出 CKEditor 5 表單，填：
   ...
   - 其餘 required 欄實機已預填、不需動：`ct-etAnnoGroup-`發布單位（系管師群組）、
     ...
     `ct-insertkey`（關鍵字）**實測留空仍可發布**。
   - **置頂**（使用者要求）：勾 `[id^="ct-top-"]` checkbox（JS click）；勾後 `ct-topEndDate/Time`
     表單**自動帶 +5 天**預設值，不需另填。
   - **附件**（`attachments`，公文目錄 `*ATTCH*` 檔）：點「附件」→「新增檔案」→
     `input[type=file].customfile[id^="attachUpload_"]`（`multiple`）一次 `send_keys` 全部路徑。
     ⚠️ 填一列後表單會**自動長出一個空的 required file 列**，會擋發布 →
     用該列的 `button.customfile-remove` 移除所有 `files.length===0` 的空列。
   - **同步顯示至**（`sync_keyword`）：點「分類」展開 → 點 `li` text 含關鍵字者。
     存查分類「研習」→「硏習資訊」（站上為異體字「硏」，用子字串 `習資訊` 比對）；其餘一律「處室公告」。
