...
│       │ 下載 zip → 複製總結/內容 → 勾列 → 點存查 → 讀檔號/填表單 → 確定存檔 →
...
2. 切到公文閱覽器新分頁,偵測公文是否含「如擬」(目前僅供 log,不影響流程)
...
13. KdApp `127.0.0.1:16888` 跳 pinCode popup:
    - 從 `id.txt` 讀 PIN,多策略嘗試填入(JS focus / click+send_keys / 純 JS setter)
