...
> 全程盡量無人值守；只有 **螢幕解鎖** 這類實體動作需要你本人(待驗證，列為下個改進點)。
> 下載專案後只要做兩件事就能用：**裝相依套件**(這讓你的AI先幫你看完專案幫你做就好) + **建立 `env.env` 帳密檔**（見下方「環境需求」）。
> 由於做公文總結需要 LLM，使用順序可在 `env.env` 的 `summarize_llm_order` 設定，預設依序:
> 1. **Antigravity `agy` CLI**(Google 帳號 OAuth、免費 — 取代已被 Google 於 2026-06-18 停用免費 OAuth 的舊 gemini CLI)
> 2. **Google AI Studio(Gemini)API key**(免費層，key 放 `~/.claude/shared-credentials.md` 或環境變數 `GEMINI_API_KEY`)
> 3. **本機 claude.ai 訂閱**(`claude -p`)
> 4. **Anthropic API key**
...
│                │           餵 LLM (antigravity agy / aistudio Gemini / claude -p / anthropic,
│                │           順序由 env.env summarize_llm_order 決定),
...
│       │ pinCode 視窗(自動填,失敗給 30s 讓使用者手動完成)→ 驗 doc_no 消失 → 寫存查標記檔
...
│                發布者+發布單位(env.env sssh_publish_unit)/置頂/附件(*ATTCH*)/同步分類(依總結 #### 多選,如 課外活動+研習資訊)→發布。
...
document_download/                — 已承辦中文件 (gitignored)
document_download_closure/        — 已存查文件 (gitignored)
...
- `pywinpty`:公文總結 antigravity backend 用 — 把 `agy` CLI 包進 ConPTY(agy 在 non-TTY 下會把回應從 stdout 丟掉,需偽終端機取回)
...
- `sssh_publish_unit`:上網公告表單「發布單位」下拉要選的單位(擇一,須與站上選項文字一致),如:圖書館/系管師群組/資訊媒體組
...
14. 驗證 `doc_no` 從「可見 `<tr>`」消失(timeout 30s,容許手動完成 PIN)
...
| 結案存查 pinCode 填不進去 | 不會中止 — 自己手動把 PIN 填完按確定,verify 階段給 30s 仍會自動寫標記檔 |
